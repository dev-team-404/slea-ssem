...
## REQ-F-A2-2: 자기평가 입력 화면

| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-A2-2-1** | 닉네임 설정 완료 후 또는 "시작하기" 클릭 시 (nickname 있고 profile 없음), 자기평가 입력 페이지로 이동해야 한다. | **M** |
| **REQ-F-A2-2-2** | 자기평가 정보(수준, 경력, 직군, 업무, 관심분야)를 입력할 수 있어야 한다. | **M** |
| **REQ-F-A2-2-3** | 모든 필수 필드가 입력되면 "완료" 버튼이 활성화되어야 한다. | **M** |
| **REQ-F-A2-2-4** | "완료" 버튼 클릭 시, user_profile 테이블에 저장하고 프로필 리뷰 페이지로 리다이렉트해야 한다. | **M** |

**수용 기준**:

- "닉네임 설정 완료 후 자기평가 입력 페이지로 자동 이동한다."
- 자기평가 정보 중 수준은 "1에서 5"로 표현하고, 숫자가 클수록 수준이 높다.
- 자기평가 정보 중 경력은 회사에 입사한 연차를 입력할 수 있도록 한다.
- 자기평가 정보 중 직군은 "SW, HW, 마케팅, 기획, 인사" 로 표기하고, 선택할 수 있다.
- 관심분야는 "AI, Backend, Frontend" 로 표기하고, 선택할 수 있다.
- "모든 필드 입력 후 '완료' 버튼이 활성화된다."
