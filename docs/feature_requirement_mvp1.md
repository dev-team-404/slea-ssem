...
| **REQ-F-A2-3** | 유효하지 않은 닉네임(길이 1-30자 범위 밖, 금칙어 포함) 입력 시 실시간으로 에러 메시지를 표시해야 한다. 한글, 영문, 숫자, 유니코드, 특수문자를 모두 지원한다. | **M** |
...
## REQ-F-A2-2: 경력 정보 입력 화면 (프로필 설정 1/2)

**Note**: 닉네임 설정 후 첫 번째 프로필 입력 단계입니다. 경력 관련 정보를 수집합니다.

| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-A2-2-1** | 닉네임 설정 완료 후, 경력 정보 입력 페이지(/career-info)로 이동해야 한다. | **M** |
| **REQ-F-A2-2-2** | 입력 항목을 명확하게 레이아웃하고 다음을 포함해야 한다: <br> - 경력(연차): 숫자 입력 필드 (정수, 0~50, 디폴트: 0) <br> - 직군: 라디오버튼 그리드 3열 (S, E, M, G, F, 디폴트: "") <br> - 담당 업무: 텍스트 입력 필드 (최대 500자, 디폴트: "") | **M** |
| **REQ-F-A2-2-3** | 모든 필드는 선택 사항이며, 입력 없이 "다음" 버튼 클릭 가능해야 한다. | **M** |
| **REQ-F-A2-2-4** | "다음" 버튼 클릭 시, 입력된 데이터를 저장하고 관심분야 및 기술 수준 입력 페이지(/self-assessment)로 이동해야 한다. | **M** |

**필드 상세 설명**:

| 필드 | 입력 타입 | 선택지 | 디폴트값 | 필수 여부 | 백엔드 API 필드 |
|------|-----------|--------|----------|----------|-----------------|
| 경력(연차) | 숫자 입력 | 정수 (0~50) | 0 | 선택 | `career` (정수) |
| 직군 | 라디오버튼 그리드 (3열) | S, E, M, G, F | "" | 선택 | `job_role` (문자열) |
...
- "닉네임 설정 완료 후 경력 정보 입력 페이지로 자동 이동한다."
- "모든 필드는 선택 사항이며, 입력 없이도 '다음' 버튼 클릭 가능하다."
- "유효하지 않은 값(경력 범위 초과 등) 입력 시 에러 메시지가 표시된다."
- "'다음' 버튼 클릭 시, 데이터가 저장되고 관심분야 및 기술 수준 입력 페이지로 이동한다."

---

## REQ-F-A2-3: 관심분야 및 기술 수준 입력 화면 (프로필 설정 2/2)

**Note**: 경력 정보 입력 후 두 번째 프로필 입력 단계입니다. 관심분야와 기술 수준을 수집합니다.

| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-A2-3-1** | 경력 정보 입력 완료 후, 관심분야 및 기술 수준 입력 페이지(/self-assessment)로 이동해야 한다. | **M** |
| **REQ-F-A2-3-2** | 입력 항목을 명확하게 레이아웃하고 다음을 포함해야 한다: <br> - 관심분야: 라디오버튼 그리드 3열 단일 선택 (AI, ML, Backend, Frontend, 디폴트: "") <br> - 본인이 생각하는 수준: 슬라이더 (1~5, 숫자가 클수록 수준이 높음, 디폴트: None) | **M** |
| **REQ-F-A2-3-3** | 기술 수준(level) 필드는 필수이며, 선택하지 않으면 "완료" 버튼이 비활성화되어야 한다. | **M** |
| **REQ-F-A2-3-4** | "완료" 버튼 클릭 시, REQ-F-A2-2에서 저장한 경력 정보와 현재 입력한 관심분야/수준 정보를 합쳐서 user_profile 테이블에 저장해야 한다. | **M** |
| **REQ-F-A2-3-5** | 저장 완료 후 프로필 리뷰 페이지(/profile-review)로 리다이렉트해야 한다. | **M** |

**필드 상세 설명**:

| 필드 | 입력 타입 | 선택지 | 디폴트값 | 필수 여부 | 백엔드 API 필드 |
|------|-----------|--------|----------|----------|-----------------|
| 관심분야 | 라디오버튼 그리드 (3열) | AI, ML, Backend, Frontend | "" | 선택 | `interests` (배열 변환 필요*) |
...
- "경력 정보 입력 완료 후 관심분야 및 기술 수준 입력 페이지로 자동 이동한다."
- "기술 수준을 선택하지 않으면 '완료' 버튼이 비활성화된다."
- "기술 수준을 선택하면 '완료' 버튼이 활성화된다."
- "유효하지 않은 값 입력 시 필드 옆에 에러 메시지가 표시된다."
- "제출 후 로딩 상태를 표시한다."
- "'완료' 버튼 클릭 시, 전체 프로필 데이터(REQ-F-A2-2 + REQ-F-A2-3)가 user_profile에 저장되고 프로필 리뷰 페이지로 이동한다."
...
- 프론트엔드: 1~5 정수
- 백엔드: "Beginner" | "Intermediate" | "Inter-Advanced" | "Advanced" | "Elite"
- 매핑: 1→Beginner, 2→Intermediate, 3→Inter-Advanced, 4→Advanced, 5→Elite
...
**Note**: 이 페이지는 헤더의 닉네임 클릭 또는 프로필 리뷰 페이지의 "수정" 버튼으로 접근 가능하며, 모든 프로필 정보를 한 페이지에서 수정할 수 있습니다.

| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-A2-Edit-1** | 헤더의 닉네임 클릭 → 드롭다운 메뉴 → "프로필 수정" 클릭 또는 프로필 리뷰 페이지의 "수정" 버튼으로 접근 가능해야 한다. | **M** |
| **REQ-F-A2-Edit-2** | 프로필 수정 페이지(/profile/edit)는 모든 프로필 정보를 한 페이지에 표시해야 한다: <br> - 닉네임 (텍스트 입력 + 중복 확인) <br> - 수준 (1~5 슬라이더) <br> - 경력(연차) (숫자 입력, 0~50) <br> - 직군 (라디오버튼 그리드 3열) <br> - 담당 업무 (텍스트 입력, max 500자) <br> - 관심분야 (라디오버튼 그리드 3열) | **M** |
| **REQ-F-A2-Edit-3** | 페이지 로드 시 현재 프로필 정보를 GET /profile/survey로 가져와 폼에 미리 채워야 한다. | **M** |
| **REQ-F-A2-Edit-4** | 닉네임 중복 확인 시, 현재 사용자의 기존 닉네임은 검증에서 제외해야 한다. (자기 자신과의 충돌 무시) | **M** |
| **REQ-F-A2-Edit-5** | "저장" 버튼 클릭 시: <br> - 닉네임이 변경된 경우: PUT /profile/nickname 호출 <br> - 자기평가 정보가 변경된 경우: PUT /profile/survey 호출 <br> - 변경 사항이 없으면 API 호출 스킵 | **M** |
| **REQ-F-A2-Edit-6** | 저장 성공 시 "저장되었습니다" 메시지를 표시하고, 프로필 리뷰 페이지(/profile-review)로 리다이렉트해야 한다. | **M** |
| **REQ-F-A2-Edit-7** | 수정된 자기평가 정보는 다음 테스트 응시 시 자동으로 반영되어야 한다. | **M** |

**필드 상세 설명**:

| 필드 | 입력 타입 | 선택지 | 디폴트값 | 필수 여부 | 백엔드 API 필드 |
|------|-----------|--------|----------|----------|-----------------|
| 닉네임 | 텍스트 입력 + 중복 확인 버튼 | - | 기존 닉네임 | 필수 | `nickname` (users 테이블) |
| 수준 | 슬라이더 (1~5) | 1, 2, 3, 4, 5 | 기존 수준 | 필수 | `level` (문자열 변환) |
| 경력(연차) | 숫자 입력 | 정수 (0~50) | 기존 경력 | 선택 | `career` (정수) |
| 직군 | 라디오버튼 그리드 (3열) | S, E, M, G, F | 기존 직군 | 선택 | `job_role` (문자열) |
| 담당 업무 | 텍스트 입력 | 자유 입력 (최대 500자) | 기존 업무 | 선택 | `duty` (문자열) |
| 관심분야 | 라디오버튼 그리드 (3열) | AI, ML, Backend, Frontend | 기존 관심분야 | 선택 | `interests` (배열 → 단일 문자열 변환) |

**라우팅**:
- 접근 경로 1: Header 닉네임 → 드롭다운 → "프로필 수정"
- 접근 경로 2: ProfileReviewPage → "수정" 버튼
- 현재: `/profile/edit`
- 저장 후: `/profile-review`

**데이터 처리**:
- 로드: GET /profile/survey → 폼 초기값 설정
- 저장:
  - 닉네임 변경 시: PUT /profile/nickname
  - 자기평가 변경 시: PUT /profile/survey
  - 둘 다 변경 시: 순차적으로 두 API 호출

**수용 기준**:

- "헤더의 닉네임 클릭 → 드롭다운 메뉴 → '프로필 수정' 클릭으로 프로필 수정 페이지에 접근할 수 있다."
- "프로필 리뷰 페이지의 '수정' 버튼으로도 접근할 수 있다."
- "프로필 수정 페이지에서 현재 모든 프로필 정보가 미리 채워져 표시된다."
- "새 닉네임 입력 후 중복 확인 시, 기존 닉네임(자기 자신)과의 충돌을 무시하고 검증한다."
- "저장 버튼 클릭 후 1초 내 '저장되었습니다' 메시지가 표시된다."
- "저장 후 프로필 리뷰 페이지로 자동 이동한다."
- "페이지 새로고침 시 변경된 정보가 반영된다."
- "변경 사항이 없으면 불필요한 API 호출을 하지 않는다."
...
| **REQ-F-B2-6** | **"다음" 버튼을 클릭할 때 현재 문항의 응답을 저장해야 한다.** 저장 완료 시 화면에 "저장됨" 표시를 해야 한다. | **M** |
...
- "**'다음' 버튼 클릭 시 응답이 저장되고 '저장됨' 메시지가 표시된다.**"
...
| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-B3-Plus-1** | **마지막 문항의 "다음" 또는 "완료" 버튼을 누르면, POST /session/{session_id}/complete 엔드포인트를 호출하여 세션을 완료해야 한다.** | **M** |
...
| **REQ-F-B4-7** | 결과 페이지에서 "문항별 해설 보기" 또는 "해설 다시 보기" 버튼을 제공하여 REQ-F-B3 (해설 화면)으로 이동할 수 있어야 한다. 사용자가 결과를 확인한 후 각 문항의 정답/오답 해설을 다시 검토할 수 있어야 한다. | **M** |
...
- "배지 다운로드 버튼이 클릭 가능하다."
- "**'해설 보기' 버튼 클릭 시 해설 화면(REQ-F-B3)으로 이동한다.**"
...
| **REQ-B-A2-Avail-2** | 닉네임 유효성 검사(길이 1-30자, 한글/영문/숫자/유니코드/특수문자 모두 허용, 금칙어 필터)를 구현해야 한다. | **M** |
...
| **REQ-B-B4-2** | 5등급 체계를 정의해야 한다: Beginner, Intermediate, Inter-Advanced, Advanced, Elite | **M** |
...
| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-B-B4-Plus-1** | **Rank-Service가 최종 등급을 산출한 후, 등급에 따라 자동으로 배지를 부여해야 한다.** 배지 종류: <br> - Beginner: "시작자 배지" <br> - Intermediate: "중급자 배지" <br> - Inter-Advanced: "중상급자 배지" <br> - Advanced: "고급자 배지" <br> - Elite: "엘리트 배지" (특수 배지) | **M** |
...
- [ ] REQ-F-B4-7: 해설 보기 버튼 (REQ-F-B3로 이동)
