...
## REQ-F-A2-Profile-Access: 헤더 닉네임 표시 및 프로필 접근

**Note**: 로그인 완료 후 (nickname != NULL), 원래 "회원가입" 버튼이 있던 헤더 위치에 사용자의 닉네임을 표시하고, 클릭 시 프로필 수정 페이지로 접근할 수 있도록 합니다.

| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-A2-Profile-Access-1** | 로그인 완료 후 (nickname != NULL), 홈화면 헤더 오른쪽 상단에 사용자의 닉네임을 표시해야 한다. "회원가입" 버튼이 있던 위치에 표시한다. | **M** |
| **REQ-F-A2-Profile-Access-2** | 헤더의 닉네임은 클릭 가능한 버튼 또는 링크 형태로 표시되어야 한다. (호버 시 시각적 피드백 제공) | **M** |
| **REQ-F-A2-Profile-Access-3** | 닉네임 클릭 시, 프로필 수정 페이지(/profile/edit)로 리다이렉트해야 한다. | **M** |
| **REQ-F-A2-Profile-Access-4** | nickname == NULL 상태에서는 "회원가입" 버튼을 표시하고, nickname != NULL 상태에서는 닉네임을 표시해야 한다. (상호 배타적) | **M** |
| **REQ-F-A2-Profile-Access-5** | 모든 페이지에서 헤더의 닉네임이 일관되게 표시되어야 한다. (전역 헤더 컴포넌트) | **M** |

**수용 기준**:

- "nickname != NULL 상태에서 헤더 오른쪽 상단에 닉네임이 표시된다."
- "nickname == NULL 상태에서는 '회원가입' 버튼이 표시되고, nickname != NULL 상태에서는 닉네임이 표시된다."
- "닉네임 클릭 시 프로필 수정 페이지로 이동한다."
- "닉네임 호버 시 시각적 피드백(색상 변경, 밑줄 등)이 제공된다."
- "모든 페이지에서 헤더의 닉네임이 일관되게 표시된다."

**UX 이점**:

- 사용자가 현재 로그인 상태와 닉네임을 한눈에 확인 가능
- 프로필 수정 페이지 접근이 명확하고 직관적
...
| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-F-A2-Edit-1** | 대시보드 또는 프로필 메뉴에서 "프로필 수정" 옵션을 제공해야 한다. 헤더의 닉네임 클릭으로도 접근 가능해야 한다. | **M** |
...
- "헤더의 닉네임 클릭으로 프로필 수정 페이지에 접근할 수 있다."
