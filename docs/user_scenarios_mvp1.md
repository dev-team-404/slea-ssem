...
- [MVP 1.0 시나리오](#mvp-10-시나리오)
  ...
  - [시나리오 0-6: 로그인 후 헤더 닉네임 표시 및 드롭다운 메뉴](#시나리오-0-6-로그인-후-헤더-닉네임-표시-및-드롭다운-메뉴)
...
| 11 | "다음" 버튼 클릭 | **users.nickname 업데이트** (nickname='태호') → 경력 정보 입력 페이지(/career-info)로 리다이렉트 | 닉네임 저장 완료 |
| 12 | 경력 정보 입력 페이지 노출 | "경력 정보를 입력해주세요" 안내 + 경력/직군/담당업무 입력 폼 | 경력 정보 입력 준비 |
| 13 | 경력 입력: "1" (연차) | 숫자 입력 | 경력 입력 |
| 14 | 직군 선택: "E" (Engineering) | 라디오버튼 그리드 3열에서 선택 | 직군 입력 |
| 15 | 담당 업무 입력: "백엔드 API 개발" | 텍스트 입력 | 담당 업무 입력 |
| 16 | "다음" 버튼 클릭 | 입력 데이터를 localStorage에 임시 저장 → 관심분야 및 기술 수준 입력 페이지(/self-assessment)로 리다이렉트 | 경력 정보 저장 완료 |
| 17 | 관심분야 및 기술 수준 입력 페이지 노출 | "관심분야와 기술 수준을 입력해주세요" 안내 + 관심분야/수준 입력 폼 | 관심분야/수준 입력 준비 |
| 18 | 관심분야 선택: "Backend" | 라디오버튼 그리드 3열에서 선택 | 관심분야 입력 |
| 19 | 기술 수준 선택: "2" (초급) | 슬라이더 선택 (1~5) | 기술 수준 입력 |
| 20 | "완료" 버튼 클릭 | localStorage 데이터 + 현재 입력 데이터 합쳐서 **user_profile 테이블에 저장** (user_id=1, level="intermediate", career=1, job_role='E', duty='백엔드 API 개발', interests=['Backend']) → 프로필 리뷰 페이지(/profile-review)로 리다이렉트 | 전체 프로필 저장 완료 |
| 21 | 프로필 리뷰 페이지 노출 | 방금 입력한 프로필 정보 표시 (읽기 전용) + "테스트 시작" 버튼 + "수정" 버튼 | 프로필 리뷰 |

**예상 결과**

- ✅ **users 테이블**: 신규 레코드 생성 (knox_id, name, dept, business_unit, email, nickname='태호')
- ✅ **user_profile 테이블**: 자기평가 정보 저장 (level="intermediate", career=1, job_role='E', duty='백엔드 API 개발', interests=['Backend'])
- ✅ JWT 토큰 발급 및 세션 유지
- ✅ 프로필 리뷰 페이지 진입 (다음: 시나리오 1 레벨 테스트)
- ✅ 3단계 온보딩 플로우: 닉네임 설정 (/set-nickname) → 경력 정보 입력 (/career-info) → 관심분야 및 기술 수준 입력 (/self-assessment) → 프로필 리뷰 (/profile-review)
...
- 이전 테스트 결과: grade=3 (Intermediate), score=72, rank='상위 28%', completedAt='2025-01-08'
...
| 2 | 홈화면 노출 | 좌측: "오늘 당신의 AI 역량을 정확하게 측정해보세요" 안내 + "레벨테스트 시작하기" 버튼<br>우측: **"나의 현재 레벨" 카드** 표시:<br>- Level 3 (숫자 표시)<br>- Intermediate 뱃지 이미지<br>- "마지막 테스트: 2025-01-08" 날짜 표시 | 홈화면 |
| 3 | "나의 현재 레벨" 카드 확인 | 백엔드: GET /api/profile/last-test-result 호출<br>Response: {hasResult: true, grade: 3, completedAt: "2025-01-08", badgeUrl: "/badges/level-3.svg"} | 카드 렌더링 완료 |
| 4 | 하단 "전체 참여자" 카드 확인 | 백엔드: GET /api/statistics/total-participants 호출<br>Response: {totalParticipants: 1234}<br>카드에 "1234명" 표시 | 통계 카드 렌더링 완료 |
| 5 | "레벨테스트 시작하기" 버튼 클릭 | 백엔드: GET /api/users/me/profile → nickname='민준', user_profile 있음 확인 → 프로필 리뷰 페이지로 리다이렉트 | 프로필 리뷰 페이지 진입 |
| 6 | 프로필 리뷰 페이지 노출 | 기존 프로필 정보 표시 (수준, 경력, 직군, 업무, 관심분야) + "테스트 시작" 버튼 + "수정" 버튼 | 프로필 리뷰 |

**예상 결과**

- ✅ 기존 사용자 AD 로그인 후 홈화면으로 즉시 이동
- ✅ **"나의 현재 레벨" 카드에 등급(3), 뱃지 이미지, 날짜(2025-01-08) 표시**
- ✅ **"전체 참여자" 카드에 참여 인원 수(1234명) 표시**
- ✅ "레벨테스트 시작하기" 클릭 시 프로필 리뷰 페이지로 이동

**검증 포인트**

- 기존 사용자 감지 로직이 정확한가?
- 홈화면에서 **"나의 현재 레벨" 카드**가 올바르게 표시되는가?
- **등급별 뱃지 이미지**가 올바르게 매핑되어 표시되는가?
- **마지막 테스트 날짜**가 "YYYY-MM-DD" 형식으로 표시되는가?
- **GET /api/profile/last-test-result** API가 정상 작동하는가?
- **GET /api/statistics/total-participants** API가 정상 작동하는가?
...
| 8 | 최종 결과 페이지 노출 | **결과 요약:** 등급 "중급", 총점 72/100, 분야별 점수 (AI: 80%, Backend: 65%), 순위 (전체 기준 145/506, 상위 28%), 등급 설명 ("중급: 업계 경험 3~5년 수준"), "해설 보기" 버튼 표시 | 결과 확인 중 |
| 9 | "해설 보기" 버튼 클릭 | 해설 화면으로 이동 (REQ-F-B3) | 해설 화면 진입 |
| 10 | 해설 화면에서 각 문항 확인 | 각 문제의 정답, 사용자 답변, 해설 + 참고 링크 제시, "다음 문항" 또는 "결과로 돌아가기" 네비게이션 제공 | 해설 상세 확인 |
| 11 | "대시보드로 이동" 클릭 (결과 페이지 또는 해설 화면에서) | 사용자 메인 대시보드 → 테스트 결과 카드 표시, "재응시" 옵션 제공 | 테스트 완료, 대시보드 진입 |
...
| 11 | 최종 결과 페이지 노출 | **결과 요약:** 등급 "중상급" (이전 "중급"에서 상향), 총점 80/100, 이전 대비 +8점 향상, 순위 "상위 22%" (이전 28%에서 상향), 이전 응답과의 비교: 개선된 분야 (LLM: 80% → 85%), 여전히 약한 분야 (Deep Learning: 70%), "해설 보기" 버튼 표시 | 결과 확인 |
| 12 | "해설 보기" 버튼 클릭 | 해설 화면으로 이동 (REQ-F-B3), 각 문제의 정답, 사용자 답변, 해설 + 참고 링크 제시 | 해설 상세 확인 |
| 13 | "결과로 돌아가기" 클릭 후 "학습 로드맵 보기" 클릭 (향후 시나리오 2.0의 학습 코디네이터 연계) | 추천 학습 자료 노출 (Deep Learning 관련 강화 학습 아이템) | 다음 단계 안내 |
...
| 14 | 최종 결과 페이지 | **결과 요약:** 등급 "엘리트 (상급+)", 종합 점수 87/100, 분야별 (LLM: 92%, RAG: 88%, Agent Architecture: 85%), 순위 "상위 5%" (전체 506명 중 25명), 특수 배지 "Agent Specialist" 부여, 등급 해설 ("엘리트: 최고 상위 5% 수준, 팀 내 기술 리더로서의 역량 입증"), "해설 보기" 버튼 표시 | 결과 확인 |
| 15 | "해설 보기" 버튼 클릭 | 해설 화면으로 이동 (REQ-F-B3), 각 문제의 정답, 사용자 답변, 해설 + 근거 자료 (참고 논문 링크, 모범 답안) 제시 | 해설 상세 확인 |
| 16 | "결과로 돌아가기" 클릭 | 최종 결과 페이지로 복귀, 특수 기능 노출: "마스터 클래스 강사 추천 프로그램" 링크 또는 "커뮤니티 기여자 등록" 옵션 | 향후 역할 제시 |
