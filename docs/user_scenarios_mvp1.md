...
## 시나리오 0-6: 로그인 후 헤더 닉네임 표시 및 프로필 수정 접근

### BDD: User Story & Acceptance Criteria

- **As a** 가입을 완료한 사용자 (nickname != NULL)
- **I want to** 로그인 후 헤더에서 내 닉네임을 확인하고, 닉네임을 클릭하여 프로필 수정 페이지에 접근한다.
...
3. 헤더의 닉네임은 **클릭 가능한 버튼/링크** 형태로 표시되어야 한다.
4. 닉네임 **호버 시 시각적 피드백** (색상 변경, 밑줄 등)을 제공해야 한다.
5. 닉네임 **클릭 시 프로필 수정 페이지**(/profile/edit)로 리다이렉트되어야 한다.
6. **모든 페이지에서** 헤더의 닉네임이 일관되게 표시되어야 한다.
...
### Scenario 0-6-2: 헤더 닉네임 클릭으로 프로필 수정 접근
...
| 2 | 헤더의 "민준" 닉네임에 마우스 호버 | 닉네임 색상 변경 또는 밑줄 표시 (클릭 가능 표시) | 호버 피드백 확인 |
| 3 | 닉네임 "민준" 클릭 | 프로필 수정 페이지(/profile/edit)로 리다이렉트 | 프로필 수정 페이지 진입 |
| 4 | 프로필 수정 페이지 노출 | 현재 닉네임 "민준" 표시 + 닉네임 입력 필드 + 자기평가 정보 수정 폼 표시 | 프로필 수정 준비 |
| 5 | 닉네임 필드에 "민준이" 입력 | 입력 필드 활성화 | 닉네임 변경 중 |
| 6 | "중복 확인" 버튼 클릭 | 백엔드 검증: "민준이" 사용 가능 → "사용 가능합니다" 메시지 | 중복 검증 완료 |
| 7 | "저장" 버튼 클릭 | users.nickname 업데이트 (nickname='민준이') + "저장되었습니다" 메시지 표시 | 변경 저장 완료 |
| 8 | 페이지 새로고침 또는 홈화면 복귀 | 헤더에 "민준이"로 업데이트된 닉네임 표시 | 변경 반영 확인 |

**예상 결과**

- ✅ 헤더의 닉네임 클릭으로 프로필 수정 페이지 접근
- ✅ 호버 시 시각적 피드백 제공
- ✅ 닉네임 변경 후 헤더에 즉시 반영
...
- 닉네임 클릭 시 프로필 수정 페이지로 정상 이동하는가?
- 닉네임 변경 후 헤더의 닉네임이 즉시 업데이트되는가?
- 헤더의 닉네임이 전역적으로 업데이트되는가?

---

### Scenario 0-6-3: 회원가입 전후 헤더 상태 변화
...
| 7 | 헤더의 "지수" 닉네임 클릭 | 프로필 수정 페이지로 이동 | 프로필 수정 페이지 진입 |
...
### Scenario 0-6-4: 프로필 수정 후 헤더 업데이트
...
| 2 | 헤더의 "유진" 클릭 | 프로필 수정 페이지로 이동 | 프로필 수정 페이지 진입 |
| 3 | 프로필 수정 페이지 노출 | 현재 닉네임 "유진" 표시 + 닉네임 입력 필드 + 자기평가 정보 표시 | 프로필 수정 준비 |
| 4 | 닉네임 필드에 "유진쓰" 입력 | 입력 필드 활성화 | 닉네임 변경 중 |
| 5 | "중복 확인" 버튼 클릭 | 백엔드 검증: "유진쓰" 사용 가능 (기존 닉네임 "유진"과의 충돌 무시) → "사용 가능합니다" 메시지 | 중복 검증 완료 |
| 6 | 자기평가 정보도 수정 (예: 수준 3 → 4로 변경) | 입력 필드 업데이트 | 자기평가 수정 중 |
| 7 | "저장" 버튼 클릭 | users.nickname 업데이트 (nickname='유진쓰') + user_profile 업데이트 (level=4) + "저장되었습니다" 메시지 표시 | 변경 저장 완료 |
| 8 | 프로필 수정 페이지 내 헤더 확인 | **헤더의 닉네임이 "유진쓰"로 즉시 업데이트**됨 (새로고침 없이) | 헤더 업데이트 확인 |
| 9 | 홈화면 복귀 | 헤더에 "유진쓰" 닉네임 표시 (모든 페이지에서 일관되게 업데이트) | 홈화면 복귀 |
